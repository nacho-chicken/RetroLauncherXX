...
# Install retrolauncher if not already installed
RL_DIR=/home/muos/retrolauncherxx
SCRIPT_DIR=/opt/muos/script
BIN_DIR=/opt/muos/extra
if [ ! -e "$RL_DIR/installed" ] && [ -d "$RL_DIR" ]; then
	cp -f "$SCRIPT_DIR/mux/frontend.sh" "$SCRIPT_DIR/mux/frontend.sh.bak"
	cp -f "$SCRIPT_DIR/mux/launch.sh" "$SCRIPT_DIR/mux/launch.sh.bak"
	cp -f "$SCRIPT_DIR/launch/lr-general.sh" "$SCRIPT_DIR/launch/lr-general.sh.bak"
	cp -f "$RL_DIR/muxtweakgen" "$BIN_DIR/muxtweakgen.bak"

	# Don't proceed if backup failed
	if [ -s "$SCRIPT_DIR/mux/frontend.sh.bak" ] && [ -s "$SCRIPT_DIR/mux/launch.sh.bak" ] && [ -s "$SCRIPT_DIR/launch/lr-general.sh.bak" ] && [ -s "$BIN_DIR/muxtweakgen.bak" ]; then
		cp -f "$RL_DIR/frontend.sh" "$SCRIPT_DIR/mux/frontend.sh"
